...
class BaseLoader(Dataset):
    ...
    def build_file_list(self, file_list_dict):
        file_list = []
        prv_list = []
        # file_list_dict의 각 항목은 PR_MODE인 경우 dict, 아니면 리스트임.
        for process_num, item in file_list_dict.items():
            if isinstance(item, dict):  # PR_MODE인 경우
                file_list.extend(item["input"])  # input_name_list (리스트)
                # 각 프로세스마다 PRV 파일은 한 개라고 가정하고 모두 동일하게 사용한다면:
                # 만약 각 입력에 대해 동일한 PRV 파일을 사용한다면 PRV 파일 경로 리스트를 입력 파일 수와 동일하게 만듭니다.
                prv_list.extend([item["prv"]] * len(item["input"]))
            else:
                file_list.extend(item)
        if not file_list:
            raise ValueError(self.dataset_name, 'No files in file list')
        import pandas as pd
        # PRV 정보가 있다면 CSV에 두 개의 컬럼으로 저장
        if prv_list:
            file_list_df = pd.DataFrame({'input_files': file_list, 'prv_files': prv_list})
        else:
            file_list_df = pd.DataFrame({'input_files': file_list})
        os.makedirs(os.path.dirname(self.file_list_path), exist_ok=True)
