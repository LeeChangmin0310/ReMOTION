...
class UBFCrPPGLoader(BaseLoader):
    ...
    def __getitem__(self, index):
        # BaseLoader의 __getitem__이 data, label, filename, chunk_id를 반환하므로 이를 먼저 받아옵니다.
        data, label, filename, chunk_id = super().__getitem__(index)
        if self.config_data.PR_MODE:
            if self.prv_files is None:
                raise AttributeError("PR_MODE is enabled but prv_files is not defined in the dataset.")
            prv_path = self.prv_files[index]
            gt_prv = np.load(prv_path)
            gt_prv = gt_prv.astype(np.float32)
            import torch
            gt_prv = torch.from_numpy(gt_prv)
            # Trainer에서는 batch[4]로 gt_prv를 기대하므로, 5개의 요소를 반환합니다.
            return data, label, filename, chunk_id, gt_prv
        else:
