    ...
    def forward(self, x):
        ...
        if math.fabs(self.theta) < 1e-8:
            ...
            if self.conv.weight.shape[2] > 1:
                kernel_diff = self.conv.weight[:, :, 0, :, :].sum(dim=(2,3)) + self.conv.weight[:, :, 2, :, :].sum(dim=(2,3))
                kernel_diff = kernel_diff[:, :, None, None, None]
                out_diff = F.conv3d(x, weight=kernel_diff, bias=self.conv.bias, stride=self.conv.stride,
...
class PhysMamba(nn.Module):
    def __init__(self, theta=0.5, drop_rate1=0.25, drop_rate2=0.5, frames=128, prv_mode=False):
        super(PhysMamba, self).__init__()
        self.prv_mode = prv_mode
    ...
    def forward(self, x):
        ...
        rPPG = x_final.view(batch, length)

        if self.prv_mode:
            wavelet_coeffs = self.wavelet_transform(rPPG)
            prv_features = torch.stack([torch.mean(coeff, dim=-1) for coeff in wavelet_coeffs], dim=1)
            prv_pred = self.prv_regressor(prv_features)
            return rPPG, prv_pred.squeeze(-1)
        return rPPG
'''
